...
export class EventsService {
  ...
  updateEventStatus(id: number, estado: string): Promise<any> {
    const headers = this.getHeaders();
    return lastValueFrom(
      this.http.put<any>(`${this.endpoint}/${id}/status`, { estado }, { headers })
    );
  }
  ...
  updateRecurrentGroup(grupo_recurrencia_id: string, data: any): Promise<any> {
    const headers = this.getHeaders();
    return lastValueFrom(
      this.http.put<any>(`${this.endpoint}/recurrentes/${grupo_recurrencia_id}`, data, { headers })
    );
  }
  ...
  deleteRecurrentGroup(grupo_recurrencia_id: string): Promise<any> {
    const headers = this.getHeaders();
    return lastValueFrom(
      this.http.delete<any>(`${this.endpoint}/recurrentes/${grupo_recurrencia_id}`, { headers })
    );
  }
}
