...
export class RecurrentGroupViewComponent {
  ...
  async loadEvents(groupId: string) {
    try {
      ...
      // 🔥 Assign elder ID from the first event in the group
      if (fetchedEvents.length > 0) {
        this.selectedElderId = Number(fetchedEvents[0].persona_mayor_id);
      }
  ...
  goBackToElderView() {
    if (this.selectedElderId) {
      this.router.navigate([`/dashboard/elderview/${this.selectedElderId}`]);
    } else {
      console.error("No elder ID selected, cannot navigate.");
    }
  }
}
