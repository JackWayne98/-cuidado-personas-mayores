...
const { checkEventConflict } = require("../models/eventoActividad.model.js");
...
const create = async (req, res) => {
  try {
    ...
    // chequeo de conflictos de eventos
    const conflicts = await EventoActividad.checkEventConflict(
      persona_mayor_id,
      fecha_inicio,
      fecha_fin
    );
    if (conflicts.length > 0) {
      return res.status(400).json({
        message:
          "Este residente ya tiene un evento que se superpone en el horario indicado.",
        conflicts,
      });
    }
...
const createRecurrentEvent = async (req, res) => {
  try {
    ...
    for (let i = 0; i < repeticiones; i++) {
      const occurrenceStart = inicio
        .add(i * intervalo_horas, "hour")
        .format("YYYY-MM-DD HH:mm:ss");
      const occurrenceEnd = fin
        .add(i * intervalo_horas, "hour")
        .format("YYYY-MM-DD HH:mm:ss");
      ...
      if (conflicts.length > 0) {
        return res.status(400).json({
          message: `Conflicto detectado en la repetición ${
            i + 1
          }: el residente ya tiene un evento que se superpone.`,
          conflicts,
          occurrenceStart,
          occurrenceEnd,
        });
